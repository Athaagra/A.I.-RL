...
class SarsaAgent:
    ...
    def select_action(self, s, policy='egreedy', epsilon=None, temp=None):
        #print(epsilon,np.random.randint(0,101,1)/100)
        if policy == 'egreedy':
            if epsilon is None:
...
def sarsa(n_timesteps, learning_rate, gamma, policy='egreedy', epsilon=None, temp=None, plot=True):
    ...
    for i in range(0,n_timesteps):
        #print(i)
        epsl=i-0/(n_timesteps - 0)#2*i/1000000
        action=pi.select_action(state,policy,epsl,temp)
        next_state,reward,done=env.step(action)
        next_action=pi.select_action(next_state,policy,epsl)
        #print(reward)
